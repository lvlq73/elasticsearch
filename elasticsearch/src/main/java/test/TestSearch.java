...
public class TestSearch {
	...
	public void searchAll()throws Exception{
		SearchRequestBuilder srb=client.prepareSearch("test-1").setTypes("purchase");
	...
	public void searchByMultiCondition()throws Exception{
		SearchRequestBuilder srb=client.prepareSearch("film").setTypes("dongzuo");
		SearchResponse sr=srb.setQuery(QueryBuilders.multiMatchQuery("战", new String[]{"title","content"}))
	...
	public void searchHighlight()throws Exception{
		...
		SearchResponse sr=srb.setQuery(QueryBuilders.matchQuery("content", "战"))
	 ...
	 */
	@Test
	public void searchMultiHighlight()throws Exception{
		SearchRequestBuilder srb=client.prepareSearch("test-1").setTypes("purchase");
		HighlightBuilder highlightBuilder=new HighlightBuilder();
		highlightBuilder.preTags("<h2><font>");
		highlightBuilder.postTags("</font></h2>");
		highlightBuilder.field("*");
		SearchResponse sr=srb.setQuery(QueryBuilders.multiMatchQuery("安安", new String[]{"vencode","venname"}))
				.highlighter(highlightBuilder)
				.setFetchSource(new String[]{"id","code","vencode","venname"},null)
				.execute()
				.actionGet();
		SearchHits hits=sr.getHits();
		for(SearchHit hit:hits){
			System.out.println(hit.getSourceAsString());
			System.out.println(hit.getHighlightFields());
			System.out.println(hit.getHighlightFields().get("venname").getFragments()[0]);
		}
	}
}
